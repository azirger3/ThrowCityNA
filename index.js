...
app.get("/temMatchesByAcctTag", async (req, res) => {
    const username = req.body.username;
    const tag = req.body.tag;
    await axios.get(`https://api.henrikdev.xyz/valorant/v1/account/${username}/${tag}`)
    .then(async (results1) => {
        const puuid = results1.data.data.puuid;
        axios.get(`https://api.henrikdev.xyz/valorant/v3/by-puuid/matches/na/${puuid}?mode=custom&size=10`)
        .then(results => {
            res.status(201).json({
                status: 'success',
                Match1: results.data.data[0].metadata.matchid,
                Match2: results.data.data[1].metadata.matchid,
                Match3: results.data.data[2].metadata.matchid,
                Match4: results.data.data[3].metadata.matchid,
                Match5: results.data.data[4].metadata.matchid,
                Match6: results.data.data[5].metadata.matchid,
                Match7: results.data.data[6].metadata.matchid,
                Match8: results.data.data[7].metadata.matchid,
                Match9: results.data.data[8].metadata.matchid,
                Match10: results.data.data[9].metadata.matchid,
                message: 'data fetched successfully',
            });
        })
        .catch(error => {
...
app.get("/matchByMatchIDCSV", (req, res) => {
    const mID = req.body.mID;
    axios.get(`https://api.henrikdev.xyz/valorant/v2/match/${mID}`)
    .then(results => {
        // Finding Agents used by A and B for NM W/L
        ...
        var csvARR = [];
        csvARR.push(returnData.topBar);
        for(stat in returnData.TeamA) {
            csvARR.push(returnData.TeamA[stat]);
        }
        for(stat in returnData.TeamB) {
            csvARR.push(returnData.TeamB[stat]);
        }
        console.log(csvARR);
